...
async def check_channels(bot, uid: int):
    ...
    For public channels  → check via get_chat_member.
    For private channels → cannot check membership, always show join button.
    """
    if not db["channels"]:
        return True, []
    missing = []
    for ch in db["channels"]:
        ...
        if ch_type == "private":
            # Cannot verify membership for private channels via Bot API
            # Always include in missing so user must click the invite link
            missing.append(ch)
        else:
            try:
...
async def _dev_info(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    txt = (
        "🤖 𝙽𝚊𝚖𝚎 : 𝙱𝙳 𝙰𝙻𝙰𝙼𝙸𝙽\n\n"
        "🏕️ 𝙳𝚛𝚎𝚊𝚖 : 𝙷𝚊𝚌𝚔𝚒𝚗𝚐 ✦︎ 𝚃𝚛𝚊𝚍𝚒𝚗𝚐 ✦ 𝙼𝚘𝚗𝚎𝚢.\n\n"
        "💥 𝚆𝚘𝚛𝚔 : 𝙽𝚘𝚝 𝙰𝚗𝚢𝚝𝚑𝚒𝚗𝚐.\n\n"
